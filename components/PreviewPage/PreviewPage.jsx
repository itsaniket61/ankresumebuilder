...
function PreviewPage({ jsonData, onDataChange }) {
    ...
    const [isMetaPaneVisible,setIsMetaPaneVisible] = useState(true);
    ...
    const components = [
        <iframe id="htmlIframe" srcDoc={htmlContent} height="100%" width="100%" key={1} />,
        ...
        <iframe src={pdfUrl}  height="100%" width="100%" key={0}/>
        :<Loader/>
    ];
    ...
    const build = async () => {
        ...
        const req = await fetch('/api/v1/build', {
            method: 'POST',
            body: JSON.stringify({ data: inputData, template }),
        });

        if (req.status === 200) {
            const res = await req.json();
            setHtmlContent(res.html);
            console.log(htmlContent, res.html);
            setLoading(false);
        }
    };

    const buildPdf = async () => {
        setIsMetaPaneVisible(false);
        setPdfUrl('');
        let html = ``;
        ...
        setLoading(true);
        data = inputData;
        const req = await fetch('/api/v1/build/PDF', {
            method: 'POST',
    ...
    }, [inputData]); // Empty dependency array ensures the effect runs only on mount and unmount
    ...
    return (
        isMobile ? (
            ...
            <>
                ...
                <div className='flex flex-col fixed bottom-1 right-1 my-1'>
                    <button
                        className={`my-1 bg-blue-700 text-white rounded-full p-2 md:p-4 focus:outline-none transition duration-300 ease-in-out shadow-lg`}
                        onClick={()=>setIsMetaPaneVisible(true)}
                    >
                        <svg xmlns="http://www.w3.org/2000/svg" height="30" width="30" viewBox="0 0 576 512" fill="white">
